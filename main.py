def xor_bytes(a, b):
    return bytes(x ^ y for x, y in zip(a, b))

def sub_bytes(state):
    s_box = [
        ,
    ]

    return bytes(s_box[b] for b in state)


def shift_rows(state, inverse=False):
    print(state)
    rows = [state[i:i + 4] for i in range(0, 16, 4)]
    shift = 1 if not inverse else -1

    for i in range(1, 4):
        rows[i] = rows[i][shift:] + rows[i][:shift]

    return bytes([rows[j][i] for i in range(4) for j in range(4)])


def mix_columns(state):
    matrix = [
        [0x02, 0x03, 0x01, 0x01],
        [0x01, 0x02, 0x03, 0x01],
        [0x01, 0x01, 0x02, 0x03],
        [0x03, 0x01, 0x01, 0x02]
    ]

    result = [0] * 16
    for i in range(4):
        for j in range(4):
            result[i + (j * 4)] = (
                (matrix[i][0] * state[j * 4])
                ^ (matrix[i][1] * state[1 + j * 4])
                ^ (matrix[i][2] * state[2 + j * 4])
                ^ (matrix[i][3] * state[3 + j * 4])
            ) % 256

    return bytes(result)

def key_expansion(key):
    rcon = [
        0x01, 0x00, 0x00, 0x00,
        0x02, 0x00, 0x00, 0x00,
        0x04, 0x00, 0x00, 0x00,
        0x08, 0x00, 0x00, 0x00,
        0x10, 0x00, 0x00, 0x00,
        0x20, 0x00, 0x00, 0x00,
        0x40, 0x00, 0x00, 0x00,
        0x80, 0x00, 0x00, 0x00,
        0x1B, 0x00, 0x00, 0x00,
        0x36, 0x00, 0x00, 0x00,
    ]

    key_schedule = list(key)
    for i in range(4, 4 * 11):
        temp = key_schedule[(i - 1) * 4:i * 4]

        if i % 4 == 0:
            temp = xor_bytes(sub_bytes(temp), bytes([rcon[i // 4 - 1], 0, 0, 0]))

        key_schedule.extend(xor_bytes(key_schedule[(i - 4) * 4:i * 4], temp))

    return key_schedule

def add_round_key(state, key_schedule, round):
    return xor_bytes(state, key_schedule[round * 16:(round + 1) * 16])

def aes_encrypt(plaintext, key):
    state = bytearray(plaintext.encode('utf-8'))
    key_schedule = key_expansion(bytearray(key.encode('utf-8')))

    state = add_round_key(state, key_schedule, 0)

    for round in range(1, 10):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, key_schedule, round)

    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, key_schedule, 10)

    return state

def aes_decrypt(ciphertext, key):
    state = bytearray(ciphertext)
    key_schedule = key_expansion(bytearray(key.encode('utf-8')))

    state = add_round_key(state, key_schedule, 10)
    state = shift_rows(state, inverse=True)
    state = sub_bytes(state, inverse=True)

    for round in range(9, 0, -1):
        state = add_round_key(state, key_schedule, round)
        state = mix_columns(state, inverse=True)
        state = shift_rows(state, inverse=True)
        state = sub_bytes(state, inverse=True)

    state = add_round_key(state, key_schedule, 0)

    return state.decode('utf-8')

# Example usage
key = "abcdefghijklmnop"  # 128-bit key
plaintext = "Hello, AES!"

# Encryption
encrypted_data = aes_encrypt(plaintext, key)
print("Encrypted:", encrypted_data)

# Decryption
decrypted_text = aes_decrypt(encrypted_data, key)
print("Decrypted:", decrypted_text)
